...
<form class="grid grid-col gap-[var(--grid-gap)]" method="POST" action="/account/addresses/?/setDefault" use:enhance={handleSubmit}>
	<input type="hidden" name="type" value={"setDefault"} />
	...
	<div class="grid grid-col gap-[var(--grid-gap)]">
		<Button style="text" type="submit">Set Default</Button>
	</div>
</form>