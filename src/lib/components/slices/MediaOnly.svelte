...
{#if image || imagePortrait || video || videoPortrait}
	<div class="relative h-full" use:inview={inViewOptions} on:inview_change={inViewChange} on:inview_animate={inViewAnimate} on:inview_init={inViewInit}>
		{#if mediaPortraitMobile && (imagePortrait || videoPortrait) && elWidth < 768}
			<div data-items={items?.length > 0 ? items.length : null} bind:clientWidth={elWidth} class="media relative min-h-full w-full overflow-hidden aspect-[var(--aspect)] h-full flex items-center justify-center {sectionPadding.toLowerCase() !== "none" || mediaRoundCorners ? "rounded-[var(--border-radius)]" : ""}" style="--aspect:{aspectRatioPortrait};">
				{#if videoPortrait}
		...
		{:else if windowWidth > 0}
			...
			<div data-items={items?.length > 0 ? items.length : null} bind:clientWidth={elWidth} class="media relative min-h-full w-full overflow-hidden aspect-[var(--aspect)] h-full flex items-center justify-center {sectionPadding.toLowerCase() !== "none" ? "rounded-[var(--border-radius)]" : ""}" style="--aspect:{aspectRatio};">
				{#if videoFallback}
